...
//         if (field === 'postalCode') {
//             // Postal code: numbers only, max 5 digits
//             value = value.replace(/\D/g, '').slice(0, 5);
//         }
//         setShippingInfo(prev => ({ ...prev, [field]: value }));
//     };

//     const isPostalValid = /^\d{5}$/.test((shippingInfo.postalCode || '').trim());
//     const postalError = (shippingInfo.postalCode || '').length > 0 && !isPostalValid;
...
//                                             <p className="text-white font-medium">ที่อยู่ที่บันทึกไว้</p>
//                                             <span className="text-gray-400 text-xs">{savedAddresses.length} รายการ</span>
//                                         </div>

//                                         <div className="space-y-3">
//                                             {addressesToShow.map((addr, i) => {
//                                                 const isSelected =
//                                                     (shippingInfo.name || '') === (addr.name || '') &&
//                                                     (shippingInfo.phone || '') === (addr.phone || '') &&
//                                                     (shippingInfo.address || '') === (addr.address || '') &&
//                                                     (shippingInfo.city || '') === (addr.city || '') &&
//                                                     (shippingInfo.postalCode || '') === (addr.postalCode || '');
...
//                                                             : 'bg-gray-900/30 border-white/10 hover:border-[#5227FF]/60'}`}
//                                                     >
//                                                         <div className="flex items-start gap-3">
//                                                             <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${isSelected ? 'bg-[#5227FF]' : 'bg-gray-700'}`}>
//                                                                 {isSelected ? (
...
//                                                             </div>
//                                                             <div className="flex-1 min-w-0">
//                                                                 <p className="text-white font-medium truncate">{addr.name || '—'} <span className="text-gray-400">• {addr.phone || '—'}</span></p>
...
//                                         {/* Additional addresses button (only when user has >1 saved address) */}
//                                         {savedAddresses.length > 1 && (
//                                             <button
//                                                 type="button"
//                                                 onClick={() => setShowAllAddresses(!showAllAddresses)}
//                                                 className="w-full py-2.5 rounded-lg border border-[#5227FF]/40 text-[#5227FF] hover:bg-[#5227FF]/10 transition flex items-center justify-center gap-2 text-sm font-medium"
//                                             >
//                                                 {showAllAddresses ? 'ซ่อนที่อยู่เพิ่มเติม' : 'ที่อยู่เพิ่มเติม'}
//                                                 <span className="text-[#5227FF]/70">({savedAddresses.length - 1})</span>
//                                                 <ChevronDown size={16} className={`transition-transform ${showAllAddresses ? 'rotate-180' : ''}`} />
//                                             </button>
//                                         )}

//                                         <p className="text-gray-500 text-xs">คลิกที่การ์ดเพื่อเลือกที่อยู่</p>
//                                     </div>
//                                 ) : null}

//                                 {/* Form */}
//                                 <div className="bg-gray-800/50 rounded-xl border border-gray-700 p-5">
//                                     <p className="text-gray-400 text-sm mb-4">กรอก/แก้ไขข้อมูลที่อยู่</p>
...
//                                                 className={`w-full bg-gray-800 text-white px-4 py-3 rounded-lg border outline-none transition ${postalError
//                                                     ? 'border-red-500 focus:border-red-500'
//                                                     : 'border-gray-600 focus:border-[#5227FF]'}`}
//                                             />
//                                             {postalError && (
//                                                 <p className="text-red-400 text-xs mt-1">กรุณากรอกรหัสไปรษณีย์ 5 หลัก (ตัวเลขเท่านั้น)</p>
//                                             )}
//                                         </div>
//                                     </div>
...
import React, { useState, useEffect } from 'react';
import { Trash2, Plus, Minus, MapPin, CreditCard, CheckCircle, QrCode, Loader2, Package, Copy, Home } from 'lucide-react';
import Stepper from './Stepper';
import Counter from './Counter';
import SlipUpload from './SlipUpload';
import { orderApi, paymentApi, userApi } from '../api';
...
// ─── Main CheckoutPage ─────────────────────────────────────────
const CheckoutPage = ({ cartItems, user, onNavigate, onOrderComplete, updateQuantity, removeFromCart }) => {
    // Navigation is handled via onNavigate prop
    ...
    const useSavedAddress = (addr, index) => {
        // Click the same address again to cancel using it
        if (selectedSavedAddressIndex === index) {
    ...
    const handleShippingChange = (field, value) => {
        // If user edits, detach from selected saved address
        if (selectedSavedAddressIndex !== null) setSelectedSavedAddressIndex(null);
        if (field === 'postalCode') {
            // Postal code: numbers only, max 5 digits
            value = value.replace(/\D/g, '').slice(0, 5);
        }
    ...
    // Saved addresses are always shown; the list is scrollable when there are many.
    ...
    if (isCompleted) {
        // Step 4: Confirmation
        const confirmedItems = (order?.items && Array.isArray(order.items) && order.items.length > 0) ? order.items : cartItems;
        return (
            <div className="min-h-screen bg-gradient-navy pt-24 pb-8 px-4 sm:px-6 flex items-center justify-center">
                <div className="w-full max-w-lg">
                    ...
                    {/* Order summary */}
                    <div className="bg-gray-800/50 rounded-2xl border border-gray-700 p-5 mb-6">
                        <h3 className="text-white font-semibold mb-4">สรุปคำสั่งซื้อ</h3>
                        ...
                        <div className="border-t border-gray-700 pt-4">
                            <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                                {confirmedItems.map((item) => {
                                    ...
                                    return (
                                        <div key={id} className="flex items-start gap-3 bg-gray-900/30 rounded-lg p-3 border border-white/10">
                                            {img ? (
                                                <img src={img} alt={name} className="w-12 h-12 rounded-lg object-cover bg-gray-900 shrink-0" />
                                            ) : (
                                                <div className="w-12 h-12 rounded-lg bg-gray-900 shrink-0" />
                                            )}
                                            <div className="flex-1 min-w-0">
                                                <p className="text-white text-sm font-medium truncate">{name}</p>
                                                <p className="text-gray-400 text-xs mt-0.5">฿{price.toLocaleString()} × {qty}</p>
                                            </div>
                                            <div className="text-white text-sm font-semibold shrink-0">฿{lineTotal.toLocaleString()}</div>
                                        </div>
                                    );
                                })}
                            </div>
                            ...
                            <div className="border-t border-gray-700 mt-4 pt-4 space-y-2">
                                ...
                                <div className="flex justify-between text-gray-400 text-sm">
                                    <span>ค่าจัดส่ง</span>
                                    <span>{shipping === 0 ? 'ฟรี' : `฿${Number(shipping).toLocaleString()}`}</span>
                                </div>
            ...
            <div className="max-w-6xl mx-auto">
                ...
                >
                    ...
                    <div>
                        <div className="flex flex-col lg:flex-row gap-6">
                            {/* Left: Address */}
                            <div className="flex-1 lg:flex-[1.45] space-y-5">
                                <h3 className="text-white font-semibold text-lg">ที่อยู่สำหรับจัดส่ง</h3>
                                    ...
                                    <div className="bg-gray-800/50 rounded-xl border border-gray-700 p-5 space-y-4">
                                        ...
                                        <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                                            {savedAddresses.map((addr, i) => {
                                                const isSelected = selectedSavedAddressIndex === i;

                                                return (
                                                    <button
                                                        key={i}
                                                        type="button"
                                                        onClick={() => useSavedAddress(addr, i)}
                                                        className={`w-full text-left rounded-xl p-4 border transition ${isSelected
                                                            ? 'bg-[#5227FF]/10 border-[#5227FF]'
                                                            : 'bg-gray-900/30 border-white/10 hover:border-[#5227FF]/60'}`}
                                                    >
                                        ...
                                        <p className="text-gray-500 text-xs">คลิกเพื่อเลือก • คลิกซ้ำเพื่อยกเลิก (เลื่อนขึ้น/ลงเพื่อดูที่อยู่ทั้งหมด)</p>
                                    </div>
                                ) : null}
                                            ...
                                            return (
                                                <div key={id} className="flex items-start gap-3 bg-gray-900/30 rounded-lg p-3 border border-white/10">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <p className="text-gray-400 text-xs mt-0.5">฿{Number(item.price || 0).toLocaleString()} × {item.quantity}</p>
                                                    </div>
                                                    <div className="text-white text-sm font-semibold shrink-0">฿{Number(lineTotal).toLocaleString()}</div>
                                                </div>
                                            );
                                        })}
                                    </div>
